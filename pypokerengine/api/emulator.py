...
class Emulator(object):
    ...
    #JBC: added the following to allow you to step to next street when reapplying a generated path
    def start_street(self, game_state):
        #call same existing method on round manager
        return RoundManager.forward_street(game_state)

    #JBC: added this to allow you to step during emulation until your next turn
    #allowing all other players to act on their own
    def step_round(self, game_state, step_action, step_amount = 0 ):
        #get the current player
        current_player_pos = game_state["next_player"]
        game_state, messages = RoundManager.apply_action(game_state, step_action, step_amount)
        
        #step to that player pos
        return self.step_to_player_turn(game_state, current_player_pos)
        
    #JBC: added this to allow you to step during emulation until your next turn
    #allowing all other players to act on their own
    def step_to_player_turn(self, game_state, player_pos ):
        #get the current player
        current_player_pos = player_pos

        #now, run again until end of street OR we encounter the next player
        mailbox = []
        next_player_pos = game_state["next_player"]
        while game_state["street"] != Const.Street.FINISHED and next_player_pos != current_player_pos:
            next_player_pos = game_state["next_player"]
            
            #if we are not the player that is stepping this round
            #then we should proceed
            if (next_player_pos != current_player_pos):
    ...
    def run_until_round_finish(self, game_state, street = Const.Street.FINISHED):
        mailbox = []
        while game_state["street"] not in [ street, Const.Street.FINISHED ]:
            next_player_pos = game_state["next_player"]
            next_player_uuid = game_state["table"].seats.players[next_player_pos].uuid
            next_player_algorithm = self.fetch_player(next_player_uuid)
            msg = MessageBuilder.build_ask_message(next_player_pos, game_state)["message"]
